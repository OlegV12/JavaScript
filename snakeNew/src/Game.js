class Game {
    ...
    doTick() {
        ...
        this.board.isNextStepToWall(this.snake.body[0]);
    ...
    /**
     * функция обновляет счетчик на игровом поле
     */
    updateCounter() {
        let counter = document.querySelector('.counter') 
        let score = this.snake.body.length - 1;
        counter.innerHTML = `Счет: ${score}`;
     ...
     */
    isGameLost() {
        if (this.board.isNextStepToBody(this.snake.body, this.snake.body[0])) {
            clearInterval(this.tickIdentifier);
